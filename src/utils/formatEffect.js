...
const rules = [
  {
    ids: ['main','activate_main','when_attacking','on_block','on_your_opponents_attack','on_ko','your_turn','opponents_turn','end_of_your_turn','on_play'],
...
export function formatEffectText(text, langCode = 'zh') {
  if (!text) return '';
  let formatted = text;
  rules.forEach(rule => {
    rule.ids.forEach(id => {
