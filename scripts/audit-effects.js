...
const KNOWN_HANDLERS = new Set([
  'ADD_DON_FROM_DECK', 'ADD_TO_HAND', 'ADD_TO_LIFE', 'ATTACH_DON', 'BOTTOM_DECK', 'CONFIRM_OPTIONAL_ACTIVATION',
  'COST_MOD', 'DECK_TO_LIFE', 'DECK_TO_TRASH', 'DEPLOY', 'DISCARD', 'DISCARD_FREE',
  'DRAW', 'FIRE_MAIN_EFFECT', 'FLIP_LIFE_FACE_UP', 'FREE_EVENT', 'GRANT_KEYWORD',
  'HAND_TO_DECK', 'HAND_TO_LIFE', 'KO', 'LIFE_TO_HAND', 'LIFE_TO_TRASH',
  'OPPONENT_DON_REST_DEFERRED', 'POWER_MOD', 'POWER_PER_DISCARD',
  'REDIRECT_ATTACK_TARGET', 'REFRESH_LOCK', 'REGISTER_ON_EVENT_TRIGGER',
  'REMAINDER_TO_TRASH', 'REST', 'RETURN_HAND', 'SEARCH', 'SELF_DEPLOY', 'UNREST', 'UNREST_DON',
]);
...
const KNOWN_CHOOSE_HANDLERS = new Set([
  ...
  'CHOOSE_DEPLOY_FROM_HAND', 'CHOOSE_DEPLOY_FROM_TRASH', 'CHOOSE_DISCARD', 'CHOOSE_DISCARD_FREE',
  'CHOOSE_DON_ATTACH_TARGET', 'CHOOSE_DON_RETURN', 'CHOOSE_DON_UNREST',
  'CHOOSE_FREE_EVENT', 'CHOOSE_GRANT_KEYWORD_TARGET', 'CHOOSE_HAND_TO_DECK',
  'CHOOSE_HAND_TO_LIFE', 'CHOOSE_KO_TARGET', 'CHOOSE_POWER_TARGET',
  'CHOOSE_REDIRECT_ATTACK_TARGET', 'CHOOSE_REFRESH_LOCK_TARGET', 'CHOOSE_REST_TARGET',
  'CHOOSE_RETURN_HAND_TARGET', 'CHOOSE_UNREST_TARGET',
]);
...
const KNOWN_MODAL_CASES = new Set([
  ...
  'CHOOSE_KO_TARGET', 'CHOOSE_POWER_TARGET', 'CHOOSE_REFRESH_LOCK_TARGET',
  'CHOOSE_REST_TARGET', 'CHOOSE_RETURN_HAND_TARGET', 'CHOOSE_UNREST_TARGET',
]);
...
function formatFilter(f) {
  ...
  if (f.cost !== undefined) p.push(`cost${f.costOp === 'gte' ? '≥' : '≤'}${f.cost}`);
...
if (!arg) {
  ...
  console.error('Usage: node --input-type=module scripts/audit-effects.js [CARD-ID | --missing]');
  process.exit(1);
} else {
