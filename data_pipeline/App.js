...
import cardPrices from "./card_prices.json";
...
import QuickController from "./components/QuickController";
...
const App = () => {
  ...
  const generateMarketShareUrl = () => {
    ...
    try {
      // 1. Format the deck string
      const deckString = entries
        .map(([id, count]) => `${count}x${id}`)
        .join(",");

      // 2. Build the object structure your useEffect expects (the "d" key)
      const shareObj = { d: deckString };

      // 3. Encode
      const jsonString = JSON.stringify(shareObj);
      const encodedData = btoa(encodeURIComponent(jsonString));

      // 4. IMPORTANT: Change '?deck=' to '?marketData='
      // This tells the useEffect to run CASE 2 and setAppMode("MARKETPLACE")
      const shareUrl = `${window.location.origin}${window.location.pathname}?marketData=${encodedData}`;

      navigator.clipboard.writeText(shareUrl).then(() => {
        alert("市場連結已複製！開啟連結後將直接進入市價視窗。");
      });
    } catch (err) {
  ...
  const deckValuation = useMemo(() => {
    ...
    Object.entries(deckList).forEach(([id, count]) => {
      if (count <= 0) return;

      // 1. Look up the price in our new flattened map
      const priceEntry = marketData[id];

      if (priceEntry) {
        // 2. Priority: Manual Price > Scraped HKD
        const unitHKD =
          parseFloat(priceEntry.price) || parseFloat(priceEntry.hkd) || 0;
        const unitJPY = parseFloat(priceEntry.jpy) || 0;
  ...
  }, [deckList, marketData]); // Ensure marketData is a dependency!

  const deckTableData = useMemo(() => {
    // 1. Define the custom order for categories
    const categoryOrder = { Leader: 1, Character: 2, Event: 3, Stage: 4 };

    return (
      Object.entries(deckList)
        ...
        .map(([id, quantity]) => {
          ...
          return {
            ...
            category: cardBase.category || "Unknown",
            cost: parseInt(cardBase.cost) || 0,
            quantity,
            unitPrice,
            totalPrice,
          };
        })
        // 2. Multi-level Sort: Category FIRST, then Cost, then Name
        .sort((a, b) => {
          ...
          // If same category, sort by Cost (Lowest to Highest)
          if (a.cost !== b.cost) return a.cost - b.cost;

          // If same cost, sort by Name
  ...
  const bulkUpdateRarity = useCallback(
    (type) => {
      // 1. Get the actual array of price data
      const allPrices = Array.isArray(cardPrices)
        ? cardPrices
        : cardPrices?.prices || [];
      ...
      setDeckList((prev) => {
        ...
        Object.entries(prev).forEach(([currentId, count]) => {
          ...
          if (type === "BASIC") {
            newDeck[baseId] = (newDeck[baseId] || 0) + count;
          } else if (type === "MAX") {
            // 2. Use the confirmed array (allPrices) to filter
            const versions = allPrices
              .filter((p) => p.id && String(p.id).startsWith(baseId))
              .sort((a, b) => {
                const getNum = (fullId) => {
                  const strId = String(fullId);
                  if (!strId.includes("_p")) return 0;
                  const parts = strId.split("_p");
            ...
            newDeck[highestId] = (newDeck[highestId] || 0) + count;
          }
        });
    ...
    [setDeckList], // Update dependencies
  );
        ...
        {appMode === "IMPORT" && (
          <div className="max-w-7xl mx-auto w-full px-4 py-8 space-y-6 animate-in fade-in zoom-in-95 duration-300">
            ...
            <div className="relative flex py-4 items-center">
              ...
              <span className="flex-shrink mx-4 text-slate-500 font-black text-xs tracking-widest uppercase">
                或
              </span>
              <div className="flex-grow border-t border-slate-800"></div>
            </div>
            ...
            <div className="p-6 bg-slate-900/80 border border-slate-700 rounded-2xl shadow-2xl">
              ...
              <div className="space-y-4">
                <textarea
                  className="w-full h-40 bg-slate-950 border border-slate-700 rounded-xl p-4 font-mono text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none text-slate-200 transition-all placeholder:text-slate-600"
                  placeholder={`在此粘貼代碼...
1xOP15-058
3xOP12-071
4xOP15-061
        ...
        <div className="max-w-7xl mx-auto w-full px-4 flex flex-col">
          ...
          {appMode === "MARKETPLACE" && (
            <div className="w-full mb-6 animate-in fade-in slide-in-from-top-4 duration-500">
              <div className="bg-slate-800/50 border border-blue-500/30 rounded-2xl p-4 backdrop-blur-sm shadow-xl">
                <div className="flex flex-row justify-between items-center gap-4">
                  ...
                  <div className="flex flex-col">
                    ...
                    <div className="flex flex-col">
                      <div className="flex items-baseline gap-2">
                        ...
                        <span className="text-slate-500 font-medium italic text-xs hidden sm:inline">
                          ≈ ¥{deckValuation.totalJPY.toLocaleString()} JPY
                        </span>
                      </div>

                      {/* Conditional Warning for Missing Prices */}
                      {deckValuation.missingCount > 0 && (
                        <p className="text-[10px] text-amber-400/80 mt-1 font-medium animate-pulse">
                          * 注意：以上總額不包括 {deckValuation.missingCount}{" "}
                          張欠缺價格的卡牌
                        </p>
                      )}
                    </div>
                    <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t border-slate-700/50">
                      <button
                        ...
                        className="flex-1 bg-amber-500/10 hover:bg-amber-500/20 border border-amber-500/50 text-amber-500 py-2 rounded-xl text-xs font-bold uppercase tracking-widest transition-all active:scale-95"
                      >
                        ✨ 我要異畫
                      </button>
                      <button
                        onClick={() => bulkUpdateRarity("BASIC")}
                        className="flex-1 bg-slate-700/30 hover:bg-slate-700/50 border border-slate-600 text-slate-400 py-2 rounded-xl text-xs font-bold uppercase tracking-widest transition-all active:scale-95"
                      >
                        ↩️ 普畫算啦
                      </button>
                    </div>
                  ...
                  {/* Right Side: Card Count Box */}
                  <div className="flex-shrink-0">
                    <div className="px-4 py-1.5 bg-slate-900/80 rounded-lg border border-slate-700 flex flex-col items-center justify-center">
                      <p className="text-[10px] text-slate-500 font-bold uppercase leading-none mb-1">
                        總卡數
                      </p>
                ...
                {selectedLeader ? (
                  ...
                  <>
                    <h2 className="text-xl font-bold text-sky-400 flex items-center gap-2">
                      ...
                      尚未選擇領航 (Select a Leader)
                    </h2>
                    <p className="text-sm text-slate-500 mt-1">
                      請先從卡片列表中點擊「領航卡」以開始編輯您的牌組。
                    </p>
                  </>
                )}
              </div>
        ...
        {appMode === "SEARCH" ? (
          <>
            <aside className="w-full lg:w-80 xl:w-96 flex-shrink-0">
              ...
              {/* Your Advanced Filters Component */}
              <section className="bg-slate-900/50 border border-slate-800 p-5 rounded-2xl backdrop-blur-md">
            ...
            <section className="flex-1">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                {filteredCards.map((card) => (
                  ...
                  >
                    {/* Slot 2: The Controller (Price Input or +/- Buttons) */}
                    {appMode === "MARKETPLACE" ? (
                      <PriceInput
                        cardId={card.id}
                        marketData={marketData}
                      ...
                      <div className="absolute bottom-2 left-0 right-0 flex justify-center px-2">
                        <QuickController
                          card={card}
                          count={deckList[card.id] || 0}
                          // When + is clicked, we pass a delta of 1
                          onAdd={(card) => updateDeckCount(card, 1)}
                          // When - is clicked, we pass a delta of -1
                          onRemove={(cardId) =>
                            updateDeckCount({ id: cardId }, -1)
                          }
                        />
                      </div>
                    )}
          ...
          <section className="w-full">
              ...
              <div className="grid grid-cols-5 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-8 xl:grid-cols-10 gap-2 md:gap-4 mb-2">
                {orderedDeck.map(({ card, count }) => (
                  <CardWrapper
                    ...
                    isCompact={true}
                    onClick={() => setSelectedCard(card)}
                    /* Slot 1: The WTB/WTS Small Badge */
                    badge={
                      appMode === "MARKETPLACE" && (
                        <div
                          onClick={(e) => {
                          ...
                          className={`
                            absolute top-0 right-0 z-30 px-2 py-1 rounded-bl-md
                      ...
                      {appMode === "MARKETPLACE" ? (
                        <div className="w-full px-1">
                          <textarea
                            placeholder="價格"
                            value={
                              marketData[card.id]?.price ||
                              marketData[card.id]?.hkd ||
                              ""
                            }
                            onClick={(e) => e.stopPropagation()}
                            onChange={(e) =>
                            ...
                            className="
                              ...
                              min-h-[34px] max-h-[50px] text-white font-mono 
                              outline-none focus:border-blue-500 shadow-inner 
                              resize-none overflow-hidden leading-tight flex items-center justify-center
                            "
                          />
                        </div>
                      ) : (
                        <QuickController
                          ...
                          onRemove={(id) => updateDeckCount({ id }, -1)}
                        />
                      )}
                    </div>
                  </CardWrapper>
            ...
            {appMode === "MARKETPLACE" && (
              <div className="max-w-7xl mx-auto w-full mt-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
                <div className="bg-slate-800/40 border border-slate-700 rounded-xl overflow-hidden shadow-2xl">
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="bg-slate-900/50 text-slate-400 text-[10px] uppercase tracking-widest border-b border-slate-700">
                        <th className="px-4 py-4 font-bold text-left">
                          卡牌資料
                        </th>{" "}
                        {/* Combined Header */}
                        <th className="px-4 py-4 font-bold text-right">單價</th>
                        <th className="px-4 py-4 font-bold text-center">
                          數量
                        </th>
                        <th className="px-4 py-4 font-bold text-right">總計</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-700/50">
                      {deckTableData.map((item) => {
                        const isMissingPrice = item.unitPrice === 0;

                        return (
                          <tr
                            key={item.id}
                            className={`
                              hover:bg-blue-500/5 transition-colors group
                              ${isMissingPrice ? "opacity-75 italic" : ""}
                            `}
                          >
                            {/* Combined Column: ID and Name */}
                            <td className="px-4 py-4">
                              <div className="flex flex-col">
                                <span className="text-[10px] font-mono text-blue-400 leading-none mb-1">
                                  {item.id}
                                </span>
                                <span className="text-sm font-medium text-slate-200 leading-tight line-clamp-1">
                                  {item.name}
                                </span>
                              </div>
                            </td>
                            ...
                            <td className="px-4 py-4 text-sm text-right">
                              {isMissingPrice ? (
                                <span className="text-amber-500/70 text-[10px] font-bold uppercase bg-amber-500/10 px-1.5 py-0.5 rounded border border-amber-500/20">
                                  N/A
                                </span>
                              ) : (
                            ...
                            {/* Quantity Column */}
                            <td className="px-4 py-4 text-sm text-center font-bold text-slate-300">
                              {item.quantity}
                            </td>
                    ...
                    <tfoot className="bg-blue-900/20 border-t-2 border-blue-500/30">
                      <tr>
                        {/* colSpan="3" covers "卡牌資料", "單價", and "數量" */}
                        <td
                          colSpan="3"
                          className="px-4 py-5 text-right text-xs font-bold text-blue-300 uppercase tracking-widest"
                        >
                          牌組總值
                        </td>

                        {/* This cell now sits exactly under the "總計" column */}
                        <td className="px-4 py-5 text-right">
                          <div className="flex flex-col items-end">
                            <span className="text-lg font-black text-white leading-none">
                              HK$ {deckValuation.totalHKD.toLocaleString()}
                            </span>
                            <span className="text-[10px] text-slate-500 italic mt-1 font-medium">
                              ≈ ¥{deckValuation.totalJPY.toLocaleString()} JPY
                            </span>
                          </div>
                        </td>
          ...
          <div className="absolute right-1 md:right-6 top-1/2 -translate-y-1/2 z-[70] flex flex-col items-center">
            ...
            <div className="absolute bottom-full mb-4 flex flex-col items-center gap-1">
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  // Always move forward by 1 in the parallel version array
                  cycleParallel(1);
                }}
                className="relative w-12 h-12 md:w-14 md:h-14 flex items-center justify-center bg-amber-500/20 hover:bg-amber-500/40 rounded-full border border-amber-500/40 text-amber-400 transition-all shadow-lg active:scale-95"
              ...
              <span className="text-xs text-amber-500/60 font-bold uppercase tracking-tighter">
                異畫
              </span>
            </div>
                ...
                <div className="flex flex-col items-end flex-shrink-0">
                  <div className="flex gap-1.5 md:gap-2">
                    <button
                      ...
                      className="w-9 h-9 md:w-10 md:h-10 bg-slate-700 hover:bg-slate-600 rounded-lg flex items-center justify-center font-bold text-xl text-white"
                    >
                      -
                    </button>
                    ...
                    <button
                      ...
                      className="w-9 h-9 md:w-10 md:h-10 bg-indigo-600 hover:bg-indigo-500 rounded-lg flex items-center justify-center font-bold text-xl text-white disabled:opacity-30"
                    >
                      +
                    </button>
                  </div>
